...
interface BlogsHeroProps {
  /** When provided, renders a 3-level breadcrumb: Home › All Blogs › blogTitle */
  blogTitle?: string;
}

// ── Component ─────────────────────────────────────────────────────────────────

const BlogsHero: React.FC<BlogsHeroProps> = ({ blogTitle }) => {
  return (
    <section
      className="relative w-full overflow-hidden mt-28"
      style={{
        background: "linear-gradient(135deg, #ede9f6 0%, #f5f0fb 40%, #fdf0f8 70%, #fce8f3 100%)",
        minHeight: "220px",
      }}
    >
      {/* ── Decorative SVGs ── */}

      {/* Planet / Saturn — top left */}
      <motion.div
      ...
      >
        <svg viewBox="0 0 260 220" fill="none" className="w-full h-full">
      ...
      >
        <svg viewBox="0 0 40 40" fill="none" className="w-full h-full">
          <polygon points="20,2 24,15 38,15 27,23 31,37 20,29 9,37 13,23 2,15 16,15" stroke="#8b7cc8" strokeWidth="1.5" fill="none" strokeLinejoin="round"/>
        </svg>
      </motion.div>
      ...
      {/* ── Main content ── */}
      <div
        className="relative z-10 px-6 sm:px-10 lg:px-16 py-12 sm:py-16 flex flex-col justify-center"
        style={{ minHeight: "inherit" }}
      >
        {/* Heading */}
        <motion.h1
          className="text-3xl sm:text-4xl lg:text-3xl font-semibold leading-tight mb-3 sm:mb-4 text-[#1a1560]"
        ...
        >
          <ol className="flex items-center gap-1 text-sm sm:text-base flex-wrap">
            ...
            {/* Home */}
            <li>
              <a href="/" className="text-gray-500 hover:text-gray-700 transition-colors duration-200 font-normal">
                Home
              </a>
            </li>
            ...
            {blogTitle ? (
              /* 3-level: Home › All Blogs › Blog Title */
              <>
                <li>
                  <a href="/blogs" className="text-gray-500 hover:text-gray-700 transition-colors duration-200 font-normal">
                    All Blogs
                  </a>
                </li>
                <li className="text-gray-400 select-none font-medium">›</li>
                <li>
                  <span
                    className="font-medium max-w-[260px] sm:max-w-sm truncate inline-block align-bottom"
                    style={{ color: "#6c5fcf" }}
                    title={blogTitle}
                  >
                    {blogTitle}
              ...
              /* 2-level: Home › All Blogs */
              <li>
                <span className="font-medium" style={{ color: "#6c5fcf" }}>
                  All Blogs
                </span>
              </li>
            )}
