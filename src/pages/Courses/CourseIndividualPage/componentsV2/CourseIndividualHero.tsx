...
interface CourseIndividualHeroProps {
  courseData: CourseData;
}

const CourseIndividualHero = ({ courseData }: CourseIndividualHeroProps) => {
  const { course_details } = courseData;
  const { course_title, course_category } = course_details;
  ...
  return (
    <section
      ...
      style={{
        background:
          "linear-gradient(135deg, #ede9f6 0%, #f5f0fb 40%, #fdf0f8 70%, #fce8f3 100%)",
        minHeight: "220px",
      }}
    >
      ...
      >
        {/* Heading */}
        <motion.h1
          className="text-3xl sm:text-4xl lg:text-4xl font-semibold leading-tight mb-3 sm:mb-4"
        ...
        >
          <ol className="flex items-center gap-1 text-sm sm:text-base flex-wrap">
            ...
            <li>
              ...
              <a  href={`/courses?category=${course_category.category_id}`}
                className="text-gray-500 hover:text-gray-700 transition-colors duration-200 font-normal"
              >
...
export default CourseIndividualHero;